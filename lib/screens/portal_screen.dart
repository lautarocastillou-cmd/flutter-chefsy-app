...
class _PortalScreenState extends State<PortalScreen> {
  ...
  RendimientoCompletoData? _rendimientoData;
  bool _cargandoRendimiento = false;

  Future<void> _cargarRendimiento() async {
    if (_cargandoRendimiento) return;
    _cargandoRendimiento = true;
    try {
      final res = await _apiService.fetchRendimientoCadete(widget.cadeteId);
      if (res != null && mounted) {
        setState(() {
          _rendimientoData = res;
        });
      }
    } catch (_) {} finally {
      if (mounted) {
          ...
          child: SafeArea(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                // ── Resumen del Turno: Viajes Entregados y Total Generado ──
                Builder(
                  builder: (context) {
